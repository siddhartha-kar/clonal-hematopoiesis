...
d <- vroom(file = "chDNMT3A_0.6info_0.002MAF_filtered.txt.gz", delim = "\t")
...
d <- d %>% mutate(BETA = BETA0/((5185/(5185+173918))*(1-(5185/(5185+173918)))))

d <- d %>% mutate(SE = SE0/((5185/(5185+173918))*(1-(5185/(5185+173918)))))
...
write.table(res, file = "dnmt3a_as_exp_misc_out.txt", row.names = F, quote = F, sep = "\t")
