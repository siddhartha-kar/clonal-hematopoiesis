...
d <- vroom(file = "chSMALL_0.6info_0.002MAF_filtered.txt.gz", delim = "\t")
...
d <- d %>% mutate(BETA = BETA0/((6154/(6154+173918))*(1-(6154/(6154+173918)))))

d <- d %>% mutate(SE = SE0/((6154/(6154+173918))*(1-(6154/(6154+173918)))))
...
write.table(res, file = "small_ch_as_exp_misc_out.txt", row.names = F, quote = F, sep = "\t")
