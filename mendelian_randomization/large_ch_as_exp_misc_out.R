...
d <- vroom(file = "chLARGE_0.6info_0.002MAF_filtered.txt.gz", delim = "\t")
...
d <- d %>% mutate(BETA = BETA0/((4049/(4049+173918))*(1-(4049/(4049+173918)))))

d <- d %>% mutate(SE = SE0/((4049/(4049+173918))*(1-(4049/(4049+173918)))))
...
ids <- vroom(file = "misc_outcomes_ids.txt", delim = "\t")
...
write.table(res, file = "large_ch_as_exp_misc_out.txt", row.names = F, quote = F, sep = "\t")
